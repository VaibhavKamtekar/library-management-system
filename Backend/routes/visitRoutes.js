...
router.post("/", async (req, res) => {
  const type = req.body.type || req.body.visitor_type;
  const { use_computer, sport } = req.body;
  ...
  try {
    ...
    let wasLastSessionAutoExited = false;
    try {
      const [lastSessions] = await dbPromise.query(
        `SELECT is_auto_exit FROM library_logs
         WHERE ${activeWhere.replace("AND exit_time IS NULL", "")}
         ORDER BY entry_time DESC, log_id DESC LIMIT 1`,
        activeParams
      );
      if (lastSessions.length > 0) {
        wasLastSessionAutoExited = lastSessions[0].is_auto_exit === 1;
      }
    } catch (dbErr) {
      console.error("Failed to query last session auto-exit status:", dbErr);
    }
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        sport: sportName,
        is_auto_exit: wasLastSessionAutoExited
      }
    });
  } catch (error) {
