...
function mapRowToStudent(row) {
  ...
  const course = String(mapped.course || mapped.department || "").trim();

  let admissionYearVal = mapped.admission_year || mapped.admissionyear || mapped.year || "";
  let admission_year = String(admissionYearVal).trim();

  if (admission_year.toUpperCase() === "FY") {
    admission_year = String(new Date().getFullYear());
  } else if (admission_year.toUpperCase() === "SY") {
    admission_year = String(new Date().getFullYear() - 1);
  } else if (!admission_year) {
    admission_year = String(new Date().getFullYear());
  }
...
router.post("/students/promote", async (req, res) => {
  ...
  try {
    ...
    if (rollNos.length > 0) {
      ...
      // No explicit rollNos supplied — promote all eligible students that match
      // the filters forwarded as query params by the frontend.
      // Reuses getStudentQueryFilters() so every filter (course, academic_year,
      // status, search) is handled consistently with the student list endpoint.
      const { whereClause, params: filterParams } = getStudentQueryFilters(req.query);
      const eligibilityClause = "status = 'active' AND academic_year IN ('FY', 'SY')";
      ...
      const [allEligibleRows] = await dbPromise.query(
        `SELECT roll_no, academic_year FROM students ${finalWhere}`,
        filterParams
      );
...
router.delete("/students/inactive", async (req, res) => {
  const { deleteLogs, deleteStudents } = req.query;
  const shouldDeleteLogs = deleteLogs === "true";
  const shouldDeleteStudents = deleteStudents === "true";

  if (!shouldDeleteLogs && !shouldDeleteStudents) {
    return res.status(400).json({ message: "No cleanup action selected." });
  }
  ...
  try {
    ...
    if (shouldDeleteStudents) {
      ...
      deletedStudents = result.affectedRows || 0;
    }
    ...
    return res.status(500).json({
      message: "Unable to complete cleanup."
    });
  }
});
